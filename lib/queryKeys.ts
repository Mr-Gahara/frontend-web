export const queryKeys = {
  ...
  pengguna: (workspace?: "outlet" | "gudang" | "keduanya" | string) =>
    workspace
      ? (["pengguna", { workspace }] as const)
      : (["pengguna"] as const),
  penggunaDetail: (id: string) => ["pengguna", "detail", id] as const,

  inventory: (locationID?: string) =>
    locationID
      ? (["inventory", { locationID }] as const)
      : (["inventory"] as const),
  inventoryDetail: (id: string) => ["inventory", id] as const,

  stockOpname: (locationID?: string) =>
    locationID
      ? (["stockOpname", { locationID }] as const)
      : (["stockOpname"] as const),
  stockOpnameDetail: (id: string) => ["stockOpname", "detail", id] as const,

  stockAdjustment: (locationID?: string) =>
    locationID
      ? (["stockAdjustment", { locationID }] as const)
      : (["stockAdjustment"] as const),
