...
import { useBuatSuratJalan } from "@/features/transfer-stok/hooks";
import { usePengajuanStok, useSetujuiPengajuan, useTolakPengajuan } from "./hooks";
import { arahPengajuanValid } from "./arah";
import { bolehBuatSuratJalan, bolehSetujuiPengajuan, bolehTolakPengajuan } from "./izin";
import { formatTanggalPengajuan } from "./tampilan";
...
const URL_DAFTAR = "/dashboard/gudang/pengajuanStok";
const URL_TRANSFER = "/dashboard/gudang/transferStok";

function badgeStatus(status: StatusPengajuan) {
  switch (status) {
    case "SUBMITTED":
    case "PENDING":
      return (
...
export default function HalamanDetailPengajuanGudang() {
  ...
  const izin = permissions ?? [];
  const bolehSetujui = bolehSetujuiPengajuan(izin);
  const bolehTolak = bolehTolakPengajuan(izin);
  const bolehSuratJalan = bolehBuatSuratJalan(izin);

  const [showConfirmApprove, setShowConfirmApprove] = useState(false);
  const [showRejectModal, setShowRejectModal] = useState(false);
  const [alasanTolak, setAlasanTolak] = useState("");

  const { data: detail, isLoading, error, refetch } = usePengajuanStok(id);

  // Dialog ditutup hanya saat berhasil; saat gagal tetap terbuka (keputusan Fase 0).
  // Detail dimuat ulang saat gagal karena stok gudang mungkin sudah berubah.
  const approveMutation = useSetujuiPengajuan(id, {
    onSuccess: () => {
      toast.success("Pengajuan Disetujui", {
        description: "Dokumen siap diproses ke tahap Surat Jalan.",
      });
      setShowConfirmApprove(false);
    },
    onError: (err) => {
      toast.error("Gagal Menyetujui", { description: pesanError(err, "Gagal menyetujui pengajuan.") });
      refetch();
    },
  });

  const rejectMutation = useTolakPengajuan(id, {
    onSuccess: () => {
    ...
    onError: (err) =>
      toast.error("Gagal Menolak", { description: pesanError(err, "Gagal menolak pengajuan.") }),
  });

  const suratJalanMutation = useBuatSuratJalan({
    onSuccess: (dibuat) => {
      toast.success("Surat Jalan Berhasil Dibuat", {
        description: "Draft pengiriman telah disiapkan.",
      });
      router.push(dibuat?.id ? `${URL_TRANSFER}/${dibuat.id}` : URL_TRANSFER);
    },
    onError: (err) =>
      toast.error("Gagal Membuat Surat Jalan", { description: pesanError(err, "Gagal membuat surat jalan.") }),
  });
  ...
  if (error || !detail) {
    return (
      <div className="flex flex-col items-center justify-center py-20">
        ...
        <Button variant="link" onClick={() => router.push(URL_DAFTAR)} className="mt-4">
          Kembali
        </Button>
      </div>
    );
  ...
  // Status menentukan tahap; izin menentukan siapa yang boleh bertindak (butir 14).
  const panelTinjau = isSubmitted && (bolehSetujui || bolehTolak);
  const panelSuratJalan = isApproved && !hasSuratJalan && bolehSuratJalan;
    ...
    <div className="flex flex-col gap-6 px-4 py-8 w-full max-w-5xl mx-auto">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 border-b border-[#0A2947]/10 pb-6">
        <div className="flex flex-col gap-4">
          <Button
            ...
            onClick={() => router.push(URL_DAFTAR)}
          >
          ...
          <div className="space-y-1.5">
            ...
            <div className="flex items-center gap-3 text-sm font-medium text-[#0A2947]/60">
              ...
              <span>•</span>
              <span>Dibuat: {formatTanggalPengajuan(detail.createdAt)}</span>
            </div>
          </div>
        </div>
      ...
      {detail.status === "REJECTED" && detail.catatanPenolakan && (
        <div className="bg-rose-50 border border-rose-200 rounded-xl p-4 flex flex-col gap-1">
          ...
          <p className="text-sm font-medium text-rose-600 pl-6">{detail.catatanPenolakan}</p>
        </div>
      )}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 items-start">
        ...
        <div className="lg:col-span-2 space-y-6">
          <div className="rounded-2xl border border-[#0A2947]/10 bg-white shadow-sm overflow-hidden">
            <div className="bg-[#F2EAE1] px-6 py-4 border-b border-[#0A2947]/10 flex items-center gap-2">
              ...
              <h2 className="font-bold text-[#0A2947]">Rincian Kebutuhan Outlet</h2>
            </div>
                        ...
                        <td className="px-6 py-4 text-center">
                          <span
                            className={cn(
                              ...
                              isKurang ? "bg-rose-50 text-rose-600" : "bg-emerald-50 text-emerald-700",
                            )}
                          >
                            {isKurang && <AlertTriangle className="w-3.5 h-3.5" />}
                            {stokTersedia}{" "}
                            <span
                              className={cn(
                                "text-xs font-sans font-medium",
                                isKurang ? "text-rose-600/70" : "text-emerald-700/70",
                              )}
                            >
                              {item.satuan}
                            </span>
          ...
          {detail.catatan && (
            <div className="rounded-2xl border border-[#0A2947]/10 bg-white p-6 shadow-sm space-y-2">
              <h3 className="font-bold text-[#0A2947] text-sm">Pesan Khusus dari Outlet:</h3>
        ...
        <div className="lg:col-span-1 space-y-6">
          <div className="rounded-2xl border border-[#0A2947]/10 bg-white p-6 shadow-sm space-y-5">
            ...
            <div className="space-y-4">
              <div className="flex flex-col gap-1 relative pl-6">
                ...
                <span className="font-bold text-[#0A2947]">{detail.dariLokasi?.nama || "-"}</span>
              </div>
              <div className="flex flex-col gap-1 relative pl-6">
                <div className="absolute left-0.75 top-1.5 w-2 h-2 rounded-full bg-[#D4A373]" />
                <span className="text-xs font-bold text-[#0A2947]/50 uppercase">Outlet Peminta</span>
                <span className="font-bold text-[#0A2947]">{detail.keLokasi?.nama || "-"}</span>
              </div>
            </div>
            ...
            <div className="border-t border-[#0A2947]/10 pt-4 mt-2 space-y-3">
              <div className="flex justify-between items-center text-sm">
                ...
                <span className="font-bold text-[#0A2947] text-right">
                  {detail.tanggalKebutuhan ? formatTanggalPengajuan(detail.tanggalKebutuhan) : "Fleksibel"}
                </span>
              </div>
              <div className="flex justify-between items-center text-sm">
              ...
              {!isSubmitted && (detail.disetujuiOleh || detail.ditolakOleh) && (
                <div className="flex justify-between items-center text-sm border-t border-[#0A2947]/5 pt-2">
                  <span className="font-bold text-[#0A2947]/60">Diproses Oleh</span>
                  <span className="font-bold text-[#0A2947] capitalize text-right">
                    {detail.disetujuiOleh?.nama || detail.ditolakOleh?.nama || "-"}
                  </span>
                </div>
              )}
            </div>
          ...
          {!arahValid && (isSubmitted || (isApproved && !hasSuratJalan)) && (
            <div className="rounded-2xl border border-rose-200 bg-rose-50 p-4 flex items-start gap-2">
              ...
              <p className="text-xs font-medium text-rose-700 leading-snug">
                Arah lokasi pengajuan ini terbalik: gudang asal dan outlet peminta tertukar.
                Persetujuan dan surat jalan dikunci agar stok tidak bergerak ke arah yang salah.
                Perbaiki data pengajuan ini lebih dulu.
              </p>
            </div>
          )}

          {/* ACTION BUTTONS: SUBMITTED, masing-masing tombol mengikuti izinnya */}
          {panelTinjau && (
            <div className="rounded-2xl border border-emerald-200 bg-emerald-50 p-6 shadow-sm flex flex-col gap-3">
              ...
              {bolehSetujui && isStockInsufficient && (
                <div className="bg-rose-50 border border-rose-200 p-3 rounded-xl mb-2 flex items-start gap-2">
                  <AlertTriangle className="w-5 h-5 text-rose-600 shrink-0 mt-0.5" />
                  <p className="text-xs font-medium text-rose-700 leading-snug">
                    Persetujuan dikunci karena stok gudang tidak mencukupi. Silakan tolak pengajuan
                    atau lakukan opname stok terlebih dahulu.
                  </p>
                </div>
              )}
          ...
          {/* ACTION BUTTONS: APPROVED tanpa surat jalan, dan pemegang create-transfer-stok */}
          {panelSuratJalan && (
            <div className="rounded-2xl border border-blue-200 bg-blue-50 p-6 shadow-sm flex flex-col gap-3">
              ...
              <Button
                onClick={() =>
                  suratJalanMutation.mutate({
                    pengajuanStokID: id,
                    tanggalKirim: new Date().toISOString(),
                  })
                }
                disabled={!arahValid || suratJalanMutation.isPending}
                className="w-full bg-blue-600 text-white hover:bg-blue-700 font-bold shadow-md cursor-pointer"
              >
                <Truck className="w-4 h-4 mr-2" />
                {suratJalanMutation.isPending ? "Memproses..." : "Buat Surat Jalan"}
              </Button>
              <p className="text-[10px] font-medium text-blue-800/60 text-center">
                Sistem otomatis menarik data item untuk divalidasi ke dalam Surat Jalan (Draft).
              </p>
            </div>
          )}
          ...
          {hasSuratJalan && (
            <div className="rounded-2xl border border-[#0A2947]/10 bg-white p-6 shadow-sm flex flex-col gap-3">
              <h3 className="text-sm font-bold text-[#0A2947] text-center mb-1">Dokumen Terkait</h3>
              <Button
                variant="outline"
                onClick={() => router.push(`${URL_TRANSFER}/${detail.transferStokID}`)}
      ...
      <AlertDialog open={showConfirmApprove} onOpenChange={setShowConfirmApprove}>
        <AlertDialogContent className="bg-[#FFFAF3] border-[#0A2947]/10">
          <AlertDialogHeader>
            <AlertDialogTitle className="text-[#0A2947]">Setujui Permintaan Outlet?</AlertDialogTitle>
            <AlertDialogDescription className="text-[#0A2947]/70 font-medium">
              Pastikan stok di gudang Anda mencukupi. Setelah disetujui, Anda harus membuat Surat
              Jalan (Transfer Stok) untuk mengirimkan barang ini secara fisik.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel
            ...
            >
              {approveMutation.isPending ? "Memproses..." : "Ya, Setujui Sekarang"}
            </AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
      </AlertDialog>
        ...
        <DialogContent className="bg-[#FFFAF3] border-[#0A2947]/10 sm:max-w-md">
          <DialogHeader>
            ...
            <DialogDescription className="text-[#0A2947]/70 font-medium pt-2">
              Berikan alasan penolakan agar staf Outlet mengetahui kendalanya (misal: stok gudang
              kosong, permintaan terlalu banyak).
            </DialogDescription>
          </DialogHeader>
          <div className="py-2">
            <Textarea
          ...
          <DialogFooter>
            ...
            <Button
              onClick={() => rejectMutation.mutate(alasanTolak.trim())}
