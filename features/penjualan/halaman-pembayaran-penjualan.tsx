...
import { usePenjualan } from "./hooks";
...
interface PropsHalamanPembayaranPenjualan {
  id: string;
}

export default function HalamanPembayaranPenjualan({ id: penjualanID }: PropsHalamanPembayaranPenjualan) {
  ...
  // permintaan berjalan: satu-satunya penjaga pembayaran ganda di web, karena
  // POST /pembayaran tidak mendukung idempotensi di backend.
  const createPembayaranMutation = useBuatPembayaran({
    onSuccess: () => {
    ...
    onError: (err) => {
      toast.error("Gagal", {
        description: pesanError(err, "Transaksi pembayaran ditolak sistem."),
      });
    },
  });
  ...
  const nominalInput = nominalDariTeks(jumlahBayarStr) ?? 0;
  ...
  if (!penjualan) {
    // Tidak ditemukan dibedakan dari gagal memuat, tanpa pengalihan otomatis
    // (sejalan dengan detail penjualan).
    return (
      <div className="mx-auto flex w-full max-w-5xl flex-col items-center gap-4 px-4 py-16 text-center">
        <p role="alert" className="text-sm font-bold text-[#0A2947]/70">
          {!errorPenjualan || isNotFound(errorPenjualan)
            ? "Penjualan tidak ditemukan."
            : pesanError(errorPenjualan, "Gagal memuat data tagihan.")}
        </p>
