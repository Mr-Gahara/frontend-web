...
import { filterServerTransfer, saringTransfer, type LingkupTujuan } from "./filter";
...
export default function HalamanDaftarPenerimaan() {
  ...
  const tujuan: LingkupTujuan | undefined = lingkup
    ? lingkup.locationID
      ? { lokasiID: lingkup.locationID }
      : { tipeLokasi: lingkup.tipeLokasi ?? "Outlet" }
    : undefined;
  const kriteria = { status: "DIKIRIM" as const, tujuan, cari: debouncedSearch, cariPada: "asal" as const };
  const {
    data: semuaTransfer = [],
    isLoading: isLoadingTransfers,
    isError,
  } = useDaftarTransferStok(lingkup ? filterServerTransfer(kriteria) : null, { refetchInterval: 30000 });
  const filteredData = saringTransfer(semuaTransfer, kriteria);
  const isLoading = cakupan.status === "memuat" || isLoadingTransfers;
  ...
  if (cakupan.status === "gagal") {
    ...
    penghalang = (
      <PesanLokasi
        ...
        isi="Lokasi kerja Anda saat ini belum dikonfigurasi, sehingga penerimaan barang tidak dapat ditampilkan. Hubungi owner atau admin."
      />
    );
  } else if (isError) {
    penghalang = (
      <PesanLokasi
        judul="Gagal Memuat Surat Jalan"
        isi="Daftar surat jalan tidak dapat dimuat. Periksa koneksi, lalu muat ulang halaman."
      />
    );
  }
