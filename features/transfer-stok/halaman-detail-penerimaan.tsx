...
import type { TransferItem, TransferStok } from "@/types/transferStok";
import { useSession } from "@/lib/auth/useSession";
import { pesanError } from "@/lib/api/error";
import { useTerimaSuratJalan, useTransferStok } from "./hooks";
import { susunPayloadTerima } from "./payload";
import { bolehTerimaSuratJalan } from "./izin";
...
/**
 * Detail penerimaan barang di ruang outlet. Form dipasang setelah detail
 * dimuat ulang (keputusan rancangan butir 8), sehingga nilai awalnya selalu
 * dari data terbaru dan tidak diisi ulang lewat effect.
 */
export default function HalamanDetailPenerimaan({ id }: { id: string }) {
  ...
  return <FormPenerimaan detail={detail} />;
}

function FormPenerimaan({ detail }: { detail: TransferStok }) {
  const router = useRouter();
  const { permissions } = useSession();
  const id = detail.id;
  const bolehTerima = bolehTerimaSuratJalan(permissions);

  // --- Form State ---
  // Kunci baris dari id bahan baku: unik per surat jalan. Menggantikan
  // crypto.randomUUID beserta cadangan Math.random.
  const [items, setItems] = useState<FormItem[]>(() =>
    detail.items.map((item, indeks) => ({
      kunci: item.bahanBaku?.id ?? `baris-${indeks}`,
  ...
  const handleNoteChange = (itemId: string, note: string) => {
    setItems((prev) =>
      prev.map((i) => (i.kunci === itemId ? { ...i, catatanItem: note } : i)),
    );
  };
  ...
  // --- Mutations ---
  // Invalidasi ada di hook (akar transfer, stok, jurnal, dan pengajuan).
  const terimaMutation = useTerimaSuratJalan(id, {
    onSuccess: () => {
      toast.success("Barang Berhasil Diterima", {
        description: "Stok Outlet telah diperbarui dan Jurnal Stok telah dicatat.",
      });
      router.push("/dashboard/outlet/inventaris/penerimaanBarang");
    },
    onError: (err) => {
      // Dialog tetap terbuka saat gagal (keputusan Fase 0).
      toast.error("Gagal Memproses Penerimaan", { description: pesanError(err, "Penerimaan gagal diproses.") });
    },
  });
                  ...
                  >
                    ...
                    <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                      ...
                      <div className="flex items-center gap-3 w-full md:w-auto">
                        <div className="flex flex-col">
                          <label htmlFor={`terima-jumlah-${item.kunci}`} className="text-[10px] font-bold text-emerald-700 uppercase block mb-1 text-right">
                            Fisik Diterima
                          </label>
                          <div className="flex items-center gap-2">
                            <Input
                              id={`terima-jumlah-${item.kunci}`}
                              type="number"
                              value={
                                item.qtyTerima === 0 &&
                                item.qtyKirim > 0 &&
                              ...
                              onChange={(e) =>
                                handleQtyChange(item.kunci, e.target.value)
                              }
                              className={cn(
                    ...
                    {isSelisih && (
                      <div className="pt-3 border-t border-rose-100 mt-1">
                        <label htmlFor={`terima-alasan-${item.kunci}`} className="text-[11px] font-bold text-rose-600 uppercase flex items-center gap-1 mb-1.5">
                          <MessageSquareWarning className="w-3.5 h-3.5" />{" "}
                          Alasan Selisih Barang{" "}
                          <span className="text-rose-500">*</span>
                        </label>
                        <Input
                          id={`terima-alasan-${item.kunci}`}
                          placeholder="Misal: 1 bungkus pecah, 2 pcs hilang di jalan..."
                          value={item.catatanItem}
                          onChange={(e) =>
                            handleNoteChange(item.kunci, e.target.value)
                          }
                          className={cn(
