...
import { useBatalSuratJalan, useKirimSuratJalan, useTransferStok } from "./hooks";
import { aksiSuratJalan } from "./izin";
...
/**
 * Detail surat jalan di ruang gudang. Tombol aksi mengikuti status dan izin
 * (aksiSuratJalan), dan dialog hanya tertutup bila aksinya berhasil
 * (keputusan Fase 0).
 */
export default function HalamanDetailTransfer({ id }: { id: string }) {
  ...
  // --- Queries ---
  const { data: detail, isLoading, refetch } = useTransferStok(id);

  // --- Mutations ---
  // Invalidasi ada di hook. Dialog ditutup hanya saat berhasil; saat gagal
  // tetap terbuka, dan detail dimuat ulang karena statusnya bisa sudah
  // diubah pihak lain.
  const kirimMutation = useKirimSuratJalan(id, {
    onSuccess: () => {
      toast.success("Barang Berhasil Dikirim", {
        description: "Stok gudang telah terpotong. Menunggu konfirmasi outlet.",
      });
      setShowConfirmKirim(false);
    },
    onError: (err) => {
      toast.error("Gagal Mengirim Barang", { description: pesanError(err, "Surat jalan gagal dikirim.") });
      void refetch();
    },
  });

  const batalMutation = useBatalSuratJalan(id, {
    onSuccess: () => {
      toast.success("Surat Jalan Dibatalkan", { description: "Dokumen pengiriman telah dibatalkan." });
      setShowConfirmBatal(false);
    },
    onError: (err) => {
      toast.error("Gagal Membatalkan", { description: pesanError(err, "Surat jalan gagal dibatalkan.") });
      void refetch();
    },
  });
  ...
  const aksi = aksiSuratJalan(detail.status, permissions);
    ...
    <div className="flex flex-col gap-6 px-4 py-8 w-full max-w-5xl mx-auto">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 items-start">
        ...
        <div className="lg:col-span-1 space-y-6">
          ...
          {(aksi.kirim || aksi.revisi || aksi.batal) && (
