...
export default function ShiftUtama({
  ...
  const filteredData = useMemo(() => {
    return dataShift.filter((shift) => {
      // Filter Nama
      const matchSearch = shift.namaShift
        .toLowerCase()
        .includes(searchQuery.toLowerCase());
      // Filter Status
      const matchStatus =
        filterStatus === "Semua" || shift.status === filterStatus;
        ...
        <div className="overflow-x-auto custom-scrollbar">
          <table className="w-full text-left text-sm whitespace-nowrap">
            ...
            <thead className="bg-[#F8F3EB] border-b border-[#041E3F]/10">
              <tr>
                <th className="p-4 font-bold text-[#041E3F]/70 uppercase tracking-wider text-xs w-[30%]">
                  Nama Shift
                </th>
                <th className="p-4 font-bold text-[#041E3F]/70 uppercase tracking-wider text-xs w-[25%]">
                  Jam Kerja
                </th>
                    ...
                    <td className="p-4">
                      <span className="font-semibold text-[#041E3F]/60">
                        {shift.toleransiTerlambat > 0 ? (
                          <span className="text-[#041E3F] font-bold">
                            {shift.toleransiTerlambat} Menit
                          </span>
                        ) : (
                          "Tepat Waktu (0 Menit)"
                        )}
                      ...
                      <DropdownMenu>
                        <DropdownMenuTrigger asChild>
                          <Button
                            variant="ghost"
                            size="icon"
                            aria-label="Buka menu" /* [PERBAIKAN] Tambahkan ini agar bisa dibaca Playwright & Screen Reader */
                        ...
                        <DropdownMenuContent
                          align="end"
                          className="w-40 bg-[#FFFAF3] border-[#041E3F]/10 rounded-xl shadow-lg"
                        >
                          <DropdownMenuItem
