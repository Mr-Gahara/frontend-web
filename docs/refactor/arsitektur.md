...
| `transfer-stok` | `api.ts`, `hooks.ts`, `payload.ts` | Pembuatan surat jalan dari pengajuan (`useBuatSuratJalan`), dipakai detail gudang pengajuan; menginvalidasi akar transfer dan pengajuan. `payload.ts` memuat `susunPayloadTerima`: setiap item surat jalan dikirim dengan `bahanBakuID` dan `qtyKirim` dari server karena backend mengganti seluruh items saat terima, jumlah 0 ditahan lewat `SERVER_TERIMA_JUMLAH_NOL`, dan item tanpa master bahan baku menahan penerimaan; dipakai halaman detail penerimaan lama sejak `dec9d01`. Sisa modul menyusul di submodul 6 |
