...
export default function EditTarifPage() {
  ...
  // Proteksi: Jika ID adalah teks "undefined", kosongkan agar tidak memicu fetch error 404
  ...
  const {
    ...
    queryKey: [queryKeys.tarif, tarifId],
    queryFn: async () => {
    ...
    enabled: !!tarifId, // Hanya request kalau ID-nya sah
  });

  // --- 2. EFFECT: PRE-FILL FORM KETIKA DATA DIDAPATKAN ---
  useEffect(() => {
    if (tarifData) {
      const mappedTipeAset =
        tarifData.tipeAsetID?.map((aset: any) =>
          typeof aset === "string" ? aset : aset._id || aset.id
        ) || [];
  ...
  if (!tarifId) {
    return (
      <div className="flex h-[80vh] w-full flex-col items-center justify-center gap-4 text-center px-4">
        ...
        <p className="text-sm font-medium" style={{ color: `${COLORS.navy}80` }}>
          Sistem mendeteksi bahwa ID tarif pada URL ini rusak atau "undefined".
          Silakan kembali ke halaman sebelumnya dan pastikan data tarif valid.
        </p>
        <Button
  ...
  if (isLoadingTarif) {
    return (
      <div className="flex h-[80vh] w-full flex-col items-center justify-center gap-4">
        ...
        />
        <p className="text-sm font-bold" style={{ color: `${COLORS.navy}80` }}>
          Memuat data tarif...
        </p>
      </div>
    );
  ...
  if (isErrorTarif) {
    return (
      <div className="flex h-[80vh] w-full flex-col items-center justify-center gap-4 text-center px-4">
        ...
        <p className="text-sm font-medium" style={{ color: `${COLORS.navy}80` }}>
          Gagal mengambil data tarif. Data mungkin sudah dihapus atau server sedang bermasalah.
        </p>
        <Button
          onClick={() => router.push("/dashboard/reservasi/tarif")}
          className="mt-4"
    ...
    <div className="flex flex-col gap-6 max-w-5xl mx-auto w-full pb-10 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <div className="space-y-1">
            <h2 className="text-xl font-bold tracking-tight" style={{ color: COLORS.navy }}>
              Edit Tarif
            </h2>
            <p className="text-sm font-medium" style={{ color: `${COLORS.navy}80` }}>
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col gap-6">
        <div
          ...
          style={{ background: COLORS.darkCream, borderColor: `${COLORS.navy}15` }}
        >
          {/* SECTION 1: INFO DASAR */}
          <div
            className="rounded-xl p-6 border shadow-sm flex flex-col gap-5"
            style={{ background: COLORS.cream, borderColor: `${COLORS.navy}10` }}
          >
            <div className="flex items-center gap-2 border-b pb-3" style={{ borderColor: `${COLORS.navy}10` }}>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
              <div className="space-y-2">
                <label className="text-sm font-bold" style={{ color: COLORS.navy }}>
              ...
              <div className="space-y-2">
                ...
                <div className="min-h-4 flex flex-col justify-start">
                  {errors.prioritas ? (
                    ...
                    <p className="text-[11px] font-medium" style={{ color: `${COLORS.navy}70` }}>
              ...
              <div
                ...
                style={{ background: `${COLORS.sage}10`, borderColor: `${COLORS.sage}30` }}
              >
                <Controller
                  name="isActive"
                  control={control}
                ...
                <div className="space-y-0.5">
                  ...
                  <p className="text-xs font-medium" style={{ color: `${COLORS.navy}80` }}>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
              ...
              <div className="space-y-2">
                ...
                <div className="relative">
                  <span className="absolute left-3 top-1/2 -translate-y-1/2 text-sm font-bold" style={{ color: `${COLORS.navy}60` }}>
                    Rp
                  </span>
                  <Input
                    type="number"
              ...
              <div className="space-y-2">
                ...
                <div className="relative">
                  ...
                  />
                  <span className="absolute right-3 top-1/2 -translate-y-1/2 text-xs font-bold" style={{ color: `${COLORS.navy}60` }}>
                    ...
                    render={({ field }) => (
                      <>
                        {HARI_MAP.map((hari) => {
                          ...
                          return (
                            <label key={hari.id} className="flex items-center gap-2 cursor-pointer">
                              <Checkbox
                                checked={isChecked}
              ...
              <div className="space-y-3">
                ...
                <div className="flex justify-between flex-col py-4 sm:flex-row sm:items-center gap-4 w-full">
                  <Controller
                    ...
                    control={control}
                    render={({ field }) => {
                      const hourVal = field.value ? field.value.split(":")[0] : "00";
                      const minuteVal = field.value ? field.value.split(":")[1] : "00";
                      return (
                        <div className="flex h-10 w-full flex-1 items-center justify-center gap-1.5 rounded-md border border-gray-200 bg-white px-3 focus-within:ring-1 focus-within:ring-gray-950 transition-shadow shadow-sm">
                          <Clock className="h-4 w-4 mr-1" style={{ color: COLORS.gold }} />
                          <input
                            type="text"
                            maxLength={2}
                            placeholder="00"
                  ...
                  />
                  <span className="text-sm font-bold hidden sm:block shrink-0" style={{ color: `${COLORS.navy}50` }}>
                    s/d
                  </span>
                  <Controller
                    name="jamSelesai"
          ...
          >
            ...
            <div className="space-y-3">
              ...
              {isLoadingAset ? (
                <div className="text-sm font-bold italic" style={{ color: `${COLORS.navy}60` }}>
                  Memuat daftar aset...
                </div>
                          ...
                          return (
                            <label
                              ...
                              className={`flex items-start gap-3 p-3 rounded-lg border cursor-pointer transition-colors ${
                                isChecked ? "bg-white shadow-sm" : "hover:bg-black/5"
                              }`}
                              style={{ borderColor: isChecked ? COLORS.navy : `${COLORS.navy}15` }}
                            >
                              <Checkbox
                                checked={isChecked}
                                onCheckedChange={(checked) => {
