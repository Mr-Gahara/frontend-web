...
const badgeStatusBayar = (status: StatusBayar) => {
  const map: Record<StatusBayar, { label: string; className: string }> = {
    PAID: {
      label: "Lunas",
      className: "bg-[#718355] text-[#FFFAF3] border-none shadow-sm", // Sage Green
    },
    UNPAID: {
      label: "Belum Bayar",
      className: "bg-[#D4A373] text-[#0A2947] border-none shadow-sm", // Mustard
    },
    PARTIAL: {
      label: "Sebagian",
      className: "bg-[#0A2947]/10 text-[#0A2947] border-none shadow-sm", // Navy Muted
    },
  };
...
const badgeStatusPenjualan = (status: StatusPenjualan) => {
  const map: Record<StatusPenjualan, { label: string; className: string }> = {
    ...
    VOID: {
      label: "Void",
      className: "bg-[#0A2947]/10 text-[#0A2947]/60 border-none shadow-sm", // Navy Muted
    },
  };
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4">
          ...
          {canPay && (
            <Button
              ...
              onClick={() =>
                router.push(`/dashboard/penjualan/${idPenjualan}/pembayaran`)
              }
            >
                  ...
                  return (
                    <tr key={validId} className="hover:bg-[#0A2947]/5 transition-colors">
                      ...
                      <td className="px-6 py-4 text-center">
                        <Badge
                          variant="outline"
                          className={`font-bold border-none shadow-sm px-2.5 py-0.5 ${
                            pay.status === "PAID"
                              ? "bg-[#718355] text-[#FFFAF3]" // Sage Green
                              : pay.status === "VOID"
                                ? "bg-[#0A2947]/10 text-[#0A2947]/60" // Muted
                                : "bg-[#D4A373] text-[#0A2947]" // Mustard
                          }`}
                        >
