...
import { MonitorSmartphone, Users, UserCheck, Clock, Loader2 } from "lucide-react";
...
export function WidgetActiveUsers({ 
  ...
  const daftarStaf = rawData?.data?.data?.daftar || rawData?.data?.daftar || rawData?.daftar || [];

  const activeUsersAPI = daftarStaf.filter((staf: any) => staf.status === "sedang_bekerja");
    ...
    <div className="rounded-xl border border-[#0A2947]/10 bg-[#F2EAE1] p-6 shadow-sm flex flex-col w-full h-full relative overflow-hidden">
      ...
      <div className="flex flex-col mb-6">
        <div className="flex items-center gap-3 mb-2 text-[#0A2947]/60">
          ...
          <span className="text-xs font-bold uppercase tracking-wider text-[#0A2947]/60">Aktif Bekerja</span>
        </div>
        <div className="flex items-end gap-2 mt-2">
          ...
          <span className="text-sm text-[#0A2947]/60 font-medium mb-1.5">online hari ini</span>
        </div>
      </div>
      ...
      <div className="flex-1 overflow-y-auto pr-2 flex flex-col gap-3 scrollbar-thin scrollbar-thumb-[#0A2947]/20">
        {isLoading ? (
           <div className="text-sm font-medium text-[#0A2947]/50 text-center mt-4">Memuat data absensi...</div>
        ) : activeUsersAPI.length === 0 ? (
           <div className="text-sm font-medium text-[#0A2947]/50 text-center mt-4">Belum ada karyawan yang absen hari ini.</div>
        ) : (
          activeUsersAPI.map((staf: any, idx: number) => {
            const matchedUser = penggunaList.find(p => (p as any).id === staf.penggunaID || p._id === staf.penggunaID);
            let roleName = "Staff"; 
            
            if (matchedUser) {
              if (typeof matchedUser.roleID === "object" && matchedUser.roleID !== null) {
                roleName = (matchedUser.roleID as any).namaRole;
              } else if (typeof matchedUser.roleID === "string") {
                const foundRole = roleList.find(r => (r as any).id === matchedUser.roleID || r._id === matchedUser.roleID);
            ...
            return (
              <div key={staf.penggunaID || idx} className="flex flex-col gap-1.5 rounded-lg border border-[#0A2947]/10 bg-[#FFFAF3] p-3 hover:border-[#0A2947]/30 transition-colors shadow-sm">
                ...
                <p className="text-xs font-semibold capitalize text-[#0A2947]/60">{roleName}</p>
              </div>
            );
          })
        )}
    ...
    <div className="rounded-xl bg-[#0A2947] p-6 shadow-sm flex flex-col justify-between text-[#FFFAF3] relative overflow-hidden w-full h-full">
      ...
      <div className="relative z-10 flex flex-col h-full justify-between gap-4">
        ...
        <div className="flex items-end gap-3">
          <span className="text-6xl lg:text-7xl font-black tracking-tighter leading-none">{penggunaList.length}</span>
          <span className="text-base text-[#FFFAF3]/70 mb-1.5 font-bold">Akun Terdaftar</span>
        </div>
      </div>
    </div>
  );
    ...
    <div className="rounded-xl border border-[#0A2947]/10 bg-[#F2EAE1] p-6 shadow-sm flex flex-col justify-between w-full h-full hover:border-[#0A2947]/20 transition-colors">
      ...
      <div className="flex items-center justify-around mt-auto pt-2">
        <div className="text-center">
          <div className="text-4xl font-black text-[#0A2947]">{appAksesCount}</div>
          <div className="text-xs font-bold text-[#0A2947]/60 mt-2 uppercase tracking-widest">Aplikasi</div>
        </div>
        <div className="w-px h-12 bg-[#0A2947]/10"></div>
        <div className="text-center">
          <div className="text-4xl font-black text-[#0A2947]">{webAksesCount}</div>
          <div className="text-xs font-bold text-[#0A2947]/60 mt-2 uppercase tracking-widest">Web</div>
        </div>
      </div>
    </div>
  );
