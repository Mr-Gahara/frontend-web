...
export default function StockOpnameDetailPage() {
  ...
  const submitMutation = useMutation({
    ...
    onSuccess: () => {
      toast.success("Berhasil Diajukan", { description: "Sesi opname telah dikunci dan diajukan ke manajer." });
      queryClient.invalidateQueries({ queryKey: queryKeys.stockOpname });
  ...
  const cancelMutation = useMutation({
    ...
    onSuccess: () => {
      toast.success("Dibatalkan", { description: "Sesi opname ini telah dibatalkan permanen." });
      queryClient.invalidateQueries({ queryKey: queryKeys.stockOpname });
  ...
  if (error || !opname) {
    return (
      <div className="flex h-[50vh] w-full flex-col items-center justify-center gap-4 text-[#0A2947]">
        ...
        <p className="font-bold">Data stok opname tidak ditemukan atau terjadi kesalahan.</p>
        <Button onClick={() => router.push("/dashboard/inventaris/stockOpname")} variant="outline">
          Kembali ke Daftar
        </Button>
      </div>
    );
      ...
      <AlertDialog open={showApproveModal} onOpenChange={setShowApproveModal}>
        <AlertDialogContent className="bg-[#FFFAF3] border-[#0A2947]/10">
          <AlertDialogHeader>
            ...
            <AlertDialogDescription className="text-[#0A2947]/70 font-medium">
              Menyetujui dokumen ini akan langsung memicu Jurnal Penyesuaian Stok. Saldo persediaan di sistem akan diubah mengikuti angka fisik yang diinput.
            </AlertDialogDescription>
            <div className="pt-4">
