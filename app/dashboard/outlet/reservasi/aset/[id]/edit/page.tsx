...
// sehingga defaultValues di useForm selalu dievaluasi dengan
// data lengkap — tidak ada race condition dengan React Query cache
// ============================================================
function EditAsetForm({
  asetData,
  tipeAsetList,
  asetId,
}: {
    ...
    <div className="mx-auto flex w-full max-w-3xl flex-col gap-6 px-4 py-8">
      <div className="flex flex-col gap-4">
        <Button
          ...
          onClick={() => router.push("/dashboard/outlet/reservasi/aset")}
        >
        ...
        <div className="space-y-1">
          ...
          <p className="text-sm font-medium text-[#0A2947]/60">
            Perbarui nama, kategori tarif, atau ubah status operasional dari
            fasilitas Anda.
          </p>
        </div>
      </div>
                ...
                <Select onValueChange={field.onChange} value={field.value}>
                  ...
                  <SelectContent className="bg-[#FFFAF3] border-[#0A2947]/10 text-[#0A2947]">
                    ...
                    <SelectItem
                      value="digunakan"
                      disabled
                      className="cursor-not-allowed text-blue-700 font-bold"
                    >
                      <div className="flex items-center">
                        <PlayCircle className="w-4 h-4 mr-2" /> Sedang Digunakan
                        (Sewa Aktif)
                      </div>
                    </SelectItem>
                  </SelectContent>
...
// ============================================================
// WRAPPER COMPONENT: menangani loading, error, dan guard
...
export default function EditAsetPage() {
  ...
  // Fetch gagal (termasuk 404) atau respons sukses tanpa data: tampilkan
  // pesan error. Sebelumnya !asetData ikut di guard loading sehingga
  // cabang ini tidak pernah tercapai dan spinner tampil selamanya.
  if (isErrorAset || !asetData) {
    return (
      <div className="flex h-[80vh] w-full flex-col items-center justify-center gap-4 text-center px-4">
        <AlertTriangle className="h-12 w-12 text-rose-500" />
        <h2 className="text-xl font-bold text-[#0A2947]">
        ...
        <p className="text-sm font-medium text-[#0A2947]/80">
          Gagal mengambil data aset. Data mungkin sudah dihapus atau server
          sedang bermasalah.
        </p>
        <Button
          onClick={() => router.push("/dashboard/outlet/reservasi/aset")}
