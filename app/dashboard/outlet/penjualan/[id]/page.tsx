...
const badgeStatusBayar = (status: StatusBayar) => {
  const map: Record<StatusBayar, { label: string; className: string }> = {
    PAID: { label: "Lunas", className: "bg-[#718355] text-[#FFFAF3] border-none shadow-sm" },
    UNPAID: { label: "Belum Bayar", className: "bg-[#D4A373] text-[#0A2947] border-none shadow-sm" },
    PARTIAL: { label: "Sebagian", className: "bg-[#0A2947]/10 text-[#0A2947] border-none shadow-sm" },
  };
  const config = map[status] ?? { label: status, className: "bg-[#0A2947]/5 text-[#0A2947]/60" };
  return (
    <Badge variant="outline" className={`${config.className} px-2.5 py-0.5 font-bold`}>
      {config.label}
    </Badge>
  );
};
...
const badgeStatusPenjualan = (status: StatusPenjualan) => {
  const map: Record<StatusPenjualan, { label: string; className: string }> = {
    FINAL: { label: "Final", className: "bg-[#718355] text-[#FFFAF3] border-none shadow-sm" },
    DRAFT: { label: "Draft", className: "bg-[#D4A373] text-[#0A2947] border-none shadow-sm" },
    VOID: { label: "Void", className: "bg-[#0A2947]/10 text-[#0A2947]/60 border-none shadow-sm" },
  };
  ...
  const finalizeMutation = useMutation({
    ...
    onError: (err: any) => {
      toast.error("Gagal Finalisasi", {
        description: err.message || "Terjadi kesalahan saat memfinalisasi penjualan.",
      });
      setShowFinalizeConfirm(false);
    },
  });
  ...
  if (loadingPenjualan) {
    return (
      <div className="flex h-[50vh] w-full flex-col items-center justify-center gap-4">
        ...
        <p className="text-sm font-bold text-[#0A2947]/60">Memuat detail transaksi...</p>
      </div>
    );
  }
    ...
    <div className="mx-auto flex w-full max-w-5xl flex-col gap-6 px-4 py-8">
      ...
      <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4">
        <div className="flex flex-col gap-3">
          ...
          <div className="space-y-1">
            <h1 className="text-2xl font-bold tracking-tight text-[#0A2947]">Detail Transaksi</h1>
            <p className="text-sm font-medium text-[#0A2947]/60">
              No. Ref: <span className="font-bold font-mono text-[#0A2947]">{penjualan.noReferensi}</span>
            </p>
          </div>
        </div>
          ...
          <div className="flex gap-2 w-full sm:w-auto">
            ...
            {canPay && (
              <Button
                ...
                onClick={() => router.push(`/dashboard/outlet/penjualan/${idPenjualan}/pembayaran`)}
              >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="rounded-2xl border border-[#0A2947]/10 bg-[#F2EAE1] p-6 sm:p-8 shadow-sm space-y-4">
          ...
          <div className="grid grid-cols-2 gap-y-5 text-sm">
            <div>
              <p className="text-[#0A2947]/60 font-bold text-xs mb-1">Tanggal & Waktu</p>
              <p className="font-semibold text-[#0A2947]">{formatTanggal(penjualan.tanggalTransaksi)}</p>
            </div>
            <div>
              <p className="text-[#0A2947]/60 font-bold text-xs mb-1">Jenis Transaksi</p>
              <p className="font-semibold text-[#0A2947] uppercase">{penjualan.jenisTransaksi}</p>
            </div>
            <div>
              <p className="text-[#0A2947]/60 font-bold text-xs mb-1">Tipe Pesanan</p>
              <p className="font-semibold text-[#0A2947] capitalize">{penjualan.jenisPenjualan}</p>
            </div>
            <div>
              <p className="text-[#0A2947]/60 font-bold text-xs mb-1">Kasir / Operator</p>
              <p className="font-semibold text-[#0A2947]">{penjualan.dataPengguna?.nama || "-"}</p>
            </div>
          </div>
        </div>
        ...
        <div className="rounded-2xl border border-[#0A2947]/10 bg-[#F2EAE1] p-6 sm:p-8 shadow-sm space-y-4">
          ...
          <div className="space-y-4 text-sm">
            <div className="flex justify-between items-start">
              <div>
                <p className="font-bold text-lg text-[#0A2947]">{penjualan.dataPelanggan?.namaPelanggan || "Umum"}</p>
                {penjualan.dataPelanggan && (
                  <Badge variant="secondary" className="mt-2 capitalize text-[10px] bg-[#D4A373] text-[#0A2947] border-none font-bold shadow-sm">
          ...
          <table className="w-full text-sm text-left">
            ...
            <tbody className="divide-y divide-[#0A2947]/5 bg-[#FFFAF3]">
              {penjualan.itemPenjualan?.map((item, idx) => (
                <tr key={idx} className="hover:bg-[#0A2947]/5 transition-colors">
                  <td className="px-6 py-4 font-bold text-[#0A2947]">{item.namaProduk}</td>
                  <td className="px-6 py-4 text-right font-semibold text-[#0A2947]">{formatRupiah(item.hargaJual)}</td>
                  <td className="px-6 py-4 text-center font-bold text-[#0A2947]">{item.jumlah}</td>
                  ...
                  <td className="px-6 py-4 text-right font-bold text-[#0A2947] font-mono">{formatRupiah(item.totalharga)}</td>
                </tr>
              ))}
            </tbody>
          </table>
        ...
        <div className="rounded-2xl border border-[#0A2947]/10 bg-[#0A2947] text-[#FFFAF3] p-6 sm:p-8 shadow-md space-y-4">
          ...
          <div className="space-y-3 text-sm">
            <div className="flex justify-between text-[#FFFAF3]/80 font-medium">
              ...
              <span className="font-mono text-[#FFFAF3]">{formatRupiah(penjualan.totalHargaProduk)}</span>
            </div>
            {penjualan.jumlahDiskonTransaksi > 0 && (
              <div className="flex justify-between text-rose-300 font-bold">
                <span>Diskon Global</span>
                <span className="font-mono">-{formatRupiah(penjualan.jumlahDiskonTransaksi)}</span>
              </div>
            )}
            {penjualan.jumlahPajakTransaksi > 0 && (
              <div className="flex justify-between text-[#FFFAF3]/80 font-bold border-t border-[#FFFAF3]/20 border-dashed pt-2 mt-1">
                ...
                <span className="font-mono">{formatRupiah(penjualan.jumlahPajakTransaksi)}</span>
              </div>
            )}
            <div className="border-t border-[#FFFAF3]/20 pt-4 mt-2 flex justify-between items-center">
              <span className="text-sm font-bold text-[#FFFAF3]">Total Tagihan</span>
            ...
            <div className="flex justify-between items-center text-[#D4A373] mt-2 pt-2 border-t border-[#FFFAF3]/10">
              ...
              <span className="font-bold font-mono">{formatRupiah(penjualan.totalDibayar)}</span>
            </div>
            {penjualan.sisaTagihan > 0 && (
              <div className="flex justify-between items-center text-rose-300 mt-2 bg-rose-500/20 p-3 rounded-xl border border-rose-500/30">
                ...
                <span className="font-black font-mono text-lg">{formatRupiah(penjualan.sisaTagihan)}</span>
              </div>
            )}
          </div>
        </div>
            ...
            <table className="w-full text-sm text-left">
              ...
              <tbody className="divide-y divide-[#0A2947]/5 bg-[#FFFAF3]">
                {pembayaranTerkait.map((pay: Pembayaran, index: number) => {
                  const validId = pay._id || (pay as any).id || `pay-fallback-${index}`;
                  return (
                    <tr key={validId} className="hover:bg-[#0A2947]/5 transition-colors">
                      <td className="px-6 py-4 font-semibold text-[#0A2947]">
                        {pay.tanggalBayar ? format(new Date(pay.tanggalBayar), "dd MMM yyyy, HH:mm") : "-"}
                      </td>
                      <td className="px-6 py-4 font-bold text-[#0A2947]">{pay.metodePembayaranID?.namaMetode || "Kasir"}</td>
                      <td className="px-6 py-4 font-medium text-[#0A2947]/60 text-xs line-clamp-1">{pay.catatan || "-"}</td>
                      <td className="px-6 py-4 text-right font-bold text-[#0A2947] font-mono">{formatRupiah(pay.jumlahBayar)}</td>
                      <td className="px-6 py-4 text-center">
                        <Badge variant="outline" className={`font-bold border-none shadow-sm px-2.5 py-0.5 ${pay.status === "PAID" ? "bg-[#718355] text-[#FFFAF3]" : pay.status === "VOID" ? "bg-[#0A2947]/10 text-[#0A2947]/60" : "bg-[#D4A373] text-[#0A2947]"}`}>
      ...
      <AlertDialog open={showFinalizeConfirm} onOpenChange={setShowFinalizeConfirm}>
        <AlertDialogContent className="bg-[#FFFAF3] border-[#0A2947]/10">
          <AlertDialogHeader>
            <AlertDialogTitle className="text-[#0A2947]">Finalisasi Invoice?</AlertDialogTitle>
            <AlertDialogDescription className="text-[#0A2947]/70 font-medium">
              Tindakan ini akan mengunci transaksi menjadi <strong className="text-[#0A2947]">FINAL</strong> dan memotong persediaan stok barang secara permanen. Apakah Anda yakin ingin melanjutkan?
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel
            ...
            >
              {finalizeMutation.isPending ? "Memproses..." : "Ya, Finalisasi Transaksi"}
            </AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
      </AlertDialog>
