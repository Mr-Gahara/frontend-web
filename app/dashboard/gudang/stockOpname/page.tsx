...
const badgeStatusOpname = (status: StatusOpname) => {
  const map: Record<StatusOpname, string> = {
    DRAFT: "bg-[#D4A373] text-[#0A2947] border-none shadow-sm",
    SUBMITTED: "bg-blue-100 text-blue-700 border-none shadow-sm",
    APPROVED: "bg-[#718355] text-[#FFFAF3] border-none shadow-sm",
    REJECTED: "bg-rose-100 text-rose-700 border-none shadow-sm",
    CANCELLED: "bg-[#0A2947]/10 text-[#0A2947]/60 border-none shadow-sm",
  };
...
export default function StockOpnameGudangPage() {
  ...
  const { data: opnameList = [], isLoading } = useQuery({
    queryKey: [...queryKeys.stockOpname.daftar(), "gudang", statusFilter],
    queryFn: async () => {
      let url = "/stockopname";
      if (statusFilter !== "ALL") {
      ...
      const allOpname = Array.isArray(fetched) ? (fetched as StockOpname[]) : [];
      
      // ISOLASI MUTLAK: Hanya kembalikan data yang lokasinya adalah Gudang
      return allOpname.filter((opn) => opn.lokasi?.tipe === "Gudang");
    },
  });
          ...
          return (
            <div className="flex justify-end">
              <Button
                ...
                // PERUBAHAN RUTE: Mengarah ke folder gudang
                onClick={() => router.push(`/dashboard/gudang/stockOpname/${targetId}`)}
              >
    ...
    <div className="mx-auto flex w-full flex-col gap-6">
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="space-y-0.5 min-w-0">
            ...
            <p className="text-xs sm:text-sm font-medium text-[#0A2947]/60 hidden sm:block">
              Kelola dan pantau dokumen perhitungan fisik stok untuk Gudang Pusat.
            </p>
          </div>
        </div>
        <div className="flex shrink-0">
          <Button
            // PERUBAHAN RUTE: Mengarah ke laman pembuatan di folder gudang
            onClick={() =>
              router.push("/dashboard/gudang/stockOpname/buatStockOpname")
            }
            size="sm"
            className="cursor-pointer gap-1.5 bg-[#0A2947] text-[#FFFAF3] hover:bg-[#0A2947]/90 font-bold shadow-sm w-full sm:w-auto"
          >
            <Plus className="h-3.5 w-3.5" />
            <span>Buat Opname Gudang</span>
          </Button>
        </div>
      </div>
      ...
      <div className="w-full overflow-x-auto rounded-2xl border border-[#0A2947]/10 bg-[#F2EAE1] p-4 sm:p-6 shadow-sm">
        <DataTable
          ...
          data={opnameList}
          loading={isLoading}
          emptyMessage="Belum ada data stok opname gudang."
