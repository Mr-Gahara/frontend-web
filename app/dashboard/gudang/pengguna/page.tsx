...
export default function PenggunaPage() {
  ...
  const { data: penggunaList = [], isLoading: penggunaLoading, error: penggunaError } = useQuery({
    queryKey: queryKeys.pengguna("gudang"),
    queryFn: async () => {
      const res = await apiClient.get<GetPenggunaResponse>("/pengguna?workspace=gudang", undefined, "pengguna");
  ...
  const savePenggunaMutation = useMutation({
    ...
    onSuccess: (_, variables) => {
      ...
      queryClient.invalidateQueries({ queryKey: queryKeys.pengguna("gudang") });
      setShowDialog(false);
    },
