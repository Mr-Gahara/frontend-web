...
export default function JurnalStokGudangPage() {
  ...
  const filteredJurnal = useMemo(() => {
    // 1. Tembok Keamanan (Anti-Ghost Cache)
    let result = Array.isArray(rawData) ? rawData : [];

    // 2. Filter Mutlak: Laman ini HANYA untuk Gudang
    result = result.filter(
      (jurnal) => jurnal.locationID?.tipe === "Gudang"
    );
    ...
    // 4. Filter Tipe Koreksi (Masuk / Keluar)
    if (tipeFilter !== "ALL") {
    ...
    // 6. Urutkan berdasarkan tanggal terbaru di atas
    result.sort(
      (a, b) => new Date(b.tanggal).getTime() - new Date(a.tanggal).getTime()
    );

    return result;
  }, [rawData, debouncedSearch, tipeFilter, alasanFilter]);
      ...
      <div className="flex flex-col gap-5 md:flex-row md:items-end md:justify-between border-b border-[#0A2947]/10 pb-6">
        <div className="space-y-1.5">
          ...
          <p className="text-sm font-medium text-[#0A2947]/60">
            Rekam jejak pergerakan keluar-masuk barang di Gudang Pusat. Data tidak dapat dimanipulasi.
          </p>
        </div>
        ...
        <div className="flex flex-col sm:flex-row gap-3 w-full md:w-auto">
          ...
          <div className="flex gap-2">
            ...
            <Select value={alasanFilter} onValueChange={setAlasanFilter}>
              ...
              <SelectContent className="bg-white border-[#0A2947]/10 text-[#0A2947]">
                ...
                <SelectItem value="Transfer Gudang" className="font-medium">Transfer Distribusi</SelectItem>
          ...
          <table className="w-full text-sm text-left whitespace-nowrap">
            <thead className="bg-[#F2EAE1] text-[11px] uppercase tracking-wider font-black text-[#0A2947]/60 border-b border-[#0A2947]/10">
              <tr>
                ...
                <th className="px-6 py-4">Alasan Jurnal</th>
                <th className="px-6 py-4 text-right">Pergerakan Fisik</th>
              ...
              {isLoading ? (
                ...
                <tr>
                  ...
                  >
                    ...
                    <span className="text-[#0A2947]/50 font-bold text-lg">Tidak Ada Catatan Ditemukan</span>
                    <span className="text-[#0A2947]/40 font-medium text-sm mt-1">
                      Belum ada pergerakan stok yang sesuai dengan filter Anda.
                    </span>
                  </td>
                </tr>
              ) : (
                filteredJurnal.map((jurnal) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      {/* KOLOM 4: PERGERAKAN ANGKA (INTI DATA) */}
